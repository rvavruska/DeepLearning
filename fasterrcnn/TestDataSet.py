...
class TestDataset(Dataset):
    def __init__(self, file):
        ...
        # /m/0k4j = Car
        #print(file)
        self.test_df = pd.read_csv(file)
        #print(self.train_df.columns)
        self.image_ids = self.test_df['ImageID'].unique()
        ...
        for i, id in enumerate(self.image_ids):
            image_id = id
            if not path.exists(f'test/data/{image_id}.jpg'):
                delete.append(i)
    ...
    def __getitem__(self, index: int):
        ...
        #print(image_id)
        bboxes = self.test_df[self.test_df['ImageID'] == image_id]
        #print(bboxes)
        

        image = cv2.imread(f'test/data/{image_id}.jpg', cv2.IMREAD_COLOR)
